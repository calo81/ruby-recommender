...
describe Recommendations::Recommender::GenericUserBasedRecommender do
  it "should return correct sorted recommendations for user" do
    ...
    rating_estimator.should_receive(:estimate).with(user, neighborhood, 3).and_return(3.0)
    rating_estimator.should_receive(:estimate).with(user, neighborhood, 4).and_return(4.0)
    rating_estimator.should_receive(:estimate).with(user, neighborhood, 5).and_return(2.0)
    rating_estimator.should_receive(:estimate).with(user, neighborhood, 6).and_return(3.5)
